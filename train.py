...
from predict import predict, load_data, normalize_data, denormalize_data


def train(mileages: list[float], prices: list[float], learning_rate: float, max_iter: int) -> dict[str, list[tuple[float, float] | float]]:
    """
    Trains a linear regression model using the gradient descent algorithm on the given dataset.
    ...
    Args:
        ...
        max_iter: The maximum number of iterations to run the gradient descent algorithm for.

    Returns:
        A dictionary containing the evolution of the theta values, learning rate, and error over time.
    """
    theta = [random.uniform(-0.1, 0.1), random.uniform(-0.1, 0.1)]
    record = {'theta': [], 'learning_rate': [], 'error': []}
    
    for i in range(max_iter):
        theta, learning_rate, error = gradient_descent(mileages, prices, theta, learning_rate)
        ...
        record['theta'].append((theta[0], theta[1]))
...
def gradient_descent(mileages: list[float], prices: list[float], theta: list[float], learning_rate: float) -> tuple[list[float], float, float]:
    """
...
def plot_results(mileages: list[float], prices: list[float], record: dict[str, list[tuple[float] | float]]):
    """
    ...
    Args:
        mileages: A list of mileage values.
        prices: A list of price values.
        record: A dictionary containing the evolution of the error, the learning rate, and theta values.
    """
    ...
    ax[0][0].plot([min_x, max_x], [predict(min_x, final_theta), predict(max_x, final_theta)], 'r')
    ...
    ax[1][1].plot(range(len(record['theta'])), [t[0] for t in record['theta']], label='Theta 0')
    ax[1][1].plot(range(len(record['theta'])), [t[1] for t in record['theta']], label='Theta 1')
